<script lang="ts">
    ...
    import { cannonicalizeExpand, deleteRecord, queryOne, save, watchOne } from "$lib/pocketbase";
    ...
    let originalSubprojects: SubprojectsRecord[] = $derived(project ? cannonicalizeExpand(project.expand.subprojects) : []);
    let originalSections: SectionsRecord[] = $derived(project ? cannonicalizeExpand(project.expand.sections) : []);
    ...
    $effect(() => {
        if(project) {
            ...
            partIdPrefix = project.part_id_prefix;
            type = project.type;
            subprojects = cannonicalizeExpand(project.expand.subprojects);
            sections = cannonicalizeExpand(project.expand.sections);
        }
    });
